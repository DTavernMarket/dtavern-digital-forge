        ...
        <div class="relative z-10 container mx-auto px-4 h-full flex items-end pb-12">
          <div class="flex items-end space-x-6">
            ...
            <div class="relative">
              ...
              />
              <div class="absolute -bottom-2 -right-2 w-8 h-8 bg-candlelight-gold rounded-full flex items-center justify-center">
            ...
            <div class="text-white">
              ...
              <div class="flex items-center space-x-6 text-sm">
                <div class="flex items-center space-x-2">
                  ...
                  <span>{{ artesao()?.avaliacao }} ({{ artesao()?.numeroAvaliacoes }} avaliações)</span>
                </div>
                <div class="flex items-center space-x-2">
          ...
          <div class="flex items-center justify-between py-4">
            <div class="flex items-center space-x-8">
              <button 
                ...
                [class]="abaAtiva() === 'produtos' ? 'text-candlelight-gold border-b-2 border-candlelight-gold' : 'text-scroll-beige hover:text-candlelight-gold'"
              ...
              <button 
                ...
                [class]="abaAtiva() === 'sobre' ? 'text-candlelight-gold border-b-2 border-candlelight-gold' : 'text-scroll-beige hover:text-candlelight-gold'"
                class="pb-2 font-medium transition-colors"
              >
                Sobre
              </button>
              <button 
                ...
                [class]="abaAtiva() === 'contato' ? 'text-candlelight-gold border-b-2 border-candlelight-gold' : 'text-scroll-beige hover:text-candlelight-gold'"
                class="pb-2 font-medium transition-colors"
              >
                Contato
              </button>
      ...
      <div class="container mx-auto px-4 py-8">
        ...
        <div *ngIf="abaAtiva() === 'produtos'" class="space-y-6">
            ...
            >
              ...
              <div class="relative aspect-square overflow-hidden">
                ...
                <div class="absolute top-3 left-3">
                  <span class="px-2 py-1 bg-candlelight-gold text-tavern-wood text-xs font-semibold rounded">
                    {{ produto.categoria }}
                  </span>
                </div>
                <div class="absolute top-3 right-3">
                  <div class="flex items-center space-x-1 bg-black/50 backdrop-blur-sm rounded px-2 py-1">
                    <svg class="w-3 h-3 text-candlelight-gold" fill="currentColor" viewBox="0 0 20 20">
                      <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/>
                    </svg>
              ...
              <div class="p-4">
                <h3 class="font-semibold text-scroll-beige mb-2 line-clamp-2">{{ produto.titulo }}</h3>
                <p class="text-scroll-beige/70 text-sm mb-3 line-clamp-2">{{ produto.descricao }}</p>
          ...
          <div *ngIf="produtosFiltrados().length === 0" class="text-center py-12">
            ...
            <p class="text-scroll-beige/70">Tente ajustar os filtros ou verifique novamente mais tarde.</p>
          </div>
        </div>
        ...
        <div *ngIf="abaAtiva() === 'sobre'" class="max-w-4xl">
          <div class="bg-tavern-wood/10 border border-brass-accent/30 rounded-xl p-8">
            <h2 class="text-2xl font-semibold text-scroll-beige mb-6">Sobre {{ artesao()?.nome }}</h2>
            ...
            <div class="prose prose-invert max-w-none">
              ...
              <div class="grid grid-cols-2 md:grid-cols-4 gap-6 mb-8">
                <div class="text-center">
                  <div class="text-3xl font-bold text-candlelight-gold">{{ artesao()?.avaliacao }}</div>
                  <div class="text-scroll-beige/70">Avaliação Média</div>
                </div>
                <div class="text-center">
                  <div class="text-3xl font-bold text-candlelight-gold">{{ artesao()?.numeroProdutos }}</div>
                  <div class="text-scroll-beige/70">Produtos</div>
                </div>
                <div class="text-center">
                  <div class="text-3xl font-bold text-candlelight-gold">{{ artesao()?.numeroSeguidores }}</div>
                  <div class="text-scroll-beige/70">Seguidores</div>
                </div>
                <div class="text-center">
                  <div class="text-3xl font-bold text-candlelight-gold">{{ artesao()?.numeroAvaliacoes }}</div>
                ...
                <div class="space-y-4">
                  ...
                  <a 
                    *ngIf="artesao()?.redesSociais?.twitter"
                    [href]="'https://twitter.com/' + artesao()?.redesSociais?.twitter?.replace('@', '')"
                    target="_blank"
                    class="flex items-center space-x-3 text-scroll-beige hover:text-candlelight-gold transition-colors"
                  >
                    <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                      <path d="M23.953 4.57a10 10 0 01-2.825.775 4.958 4.958 0 002.163-2.723c-.951.555-2.005.1-2.827-.384a4.92 4.92 0 00-8.384 4.482C7.69 8.095 4.067 6.13 1.64 3.162a4.822 4.822 0 00-.666 2.475c0 1.71.87 3.213 2.188 4.096a4.904 4.904 0 01-2.228-.616v.06a4.923 4.923 0 003.946 4.827 4.996 4.996 0 01-2.212.085 4.936 4.936 0 004.604 3.417 9.867 9.867 0 01-6.102 2.105c-.39 0-.779-.023-1.17-.067a13.995 13.995 0 007.557 2.209c9.053 0 13.998-7.496 13.998-13.985 0-.21 0-.42-.015-.63A9.935 9.935 0 0024 4.59z"/>
                    </svg>
                  ...
                  <a 
                    *ngIf="artesao()?.redesSociais?.instagram"
                    [href]="'https://instagram.com/' + artesao()?.redesSociais?.instagram?.replace('@', '')"
                    target="_blank"
                    class="flex items-center space-x-3 text-scroll-beige hover:text-candlelight-gold transition-colors"
                  >
                    <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
...
export class LojaArtesaoComponent implements OnInit{
  ...
  produtosFiltrados = computed(() => {
    ...
    switch (this.ordenacao()) {
      ...
      case 'recentes':
      default:
        produtos = produtos.sort((a, b) => new Date(b.dataCriacao).getTime() - new Date(a.dataCriacao).getTime());
        break;
    }
  ...
  private carregarArtesao(dominio: string) {
    ...
    if (artesao) {
      ...
      this.carregarProdutos(artesao.uuid);
    } else {
